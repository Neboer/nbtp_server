...
class full_upload_task {
  CURLM *multi_handle;
  shared_ptr<string>
      target_url; // 这个string并没有被多线程访问，要限制这个访问仅限于此线程中。
  function<CURL *(unique_ptr<nbtp_chunk>)>
      convert_to_uploadable_object; // 将一个二进制数据块变成curl_easy_handle，以便于加入multi传输过程。
  full_upload_task(
      string *target_url,
...
public:
  ...
  indexed_key_str
  wait_for_an_result(); // 阻塞的等待一个上传任务结束，内部使用curl poll来实现。
};

...
int full_upload_task::add_task(unique_ptr<nbtp_chunk> data_wait_for_upload) {
  auto curl_obj = convert_to_uploadable_object(move(data_wait_for_upload));
...
class full_upload_task {
  ...
  full_upload_task(string target_url,
                   function<curl_mime *(nbtp_chunk *, CURL *)> wrap_func) {}

private:
  static size_t
  server_response_collector(void *contents, size_t size, size_t nmemb,
                            void *target_result_block_write_location) {
    size_t real_size = size * nmemb;
    nbtp_chunk *target_chunk = (nbtp_chunk *)target_result_block_write_location;
    target_chunk->insert(target_chunk->end(), (unsigned char *)contents,
...
public:
  promise<string *> *add_task(nbtp_chunk *data_wait_for_upload) {
    ...
    auto task_info =
        new communicate_message(promise_containing_result, server_response);
    curl_easy_setopt(current_easy_handle, CURLOPT_MIMEPOST,
                     wrap_func(data_wait_for_upload, current_easy_handle));
    curl_easy_setopt(current_easy_handle, CURLOPT_URL, target_url->c_str());
    curl_easy_setopt(current_easy_handle, CURLOPT_WRITEFUNCTION,
                     server_response_collector);

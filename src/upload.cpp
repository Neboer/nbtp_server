...
using namespace std;
void generator_manager(function<indexed_nbtp_chunk()> generator, ThreadsafeQueue<indexed_nbtp_chunk> *generate_queue) {
...
void transformer_manager(
    ...
    function<indexed_key_str()> wait_for_an_result,    // 阻塞等待之前安排的所有上传任务
    ThreadsafeQueue<indexed_nbtp_chunk> *waiting_upload_queue, function<void(indexed_key_str)> key_sender, int concurrent_upload_proc_count) {

  indexed_nbtp_chunk current_upload_task = waiting_upload_queue->pop();
  int current_upload_count = 1;
  while (current_upload_count > 0) { // 只要还有正在上传的任务，主循环就不能停下。
    while (current_upload_count < concurrent_upload_proc_count && current_upload_task.index) {
      current_upload_count = start_upload_bk(move(current_upload_task)); // 在下面的阻塞等待中，一旦有一个上传完成，立刻准备拿出新的开始上传，同时也可以等之前的转换一段时间。
      current_upload_task = waiting_upload_queue->pop();
    }
    indexed_key_str key = wait_for_an_result(); // 等待一个上传结果，如果有则立即返回
    key_sender(move(key));
  }
}
